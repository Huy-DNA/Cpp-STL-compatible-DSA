...
template <typename Elem_T, typename Comp_T = std::less<const Elem_T&>,
          typename = std::enable_if_t<std::is_trivially_copyable<Elem_T>::value, void>>
class RedBlackTree {
    friend class ConstBiIter<Elem_T, Comp_T>;
    typedef std::size_t size_t;
    typedef ConstBiIter<Elem_T, Comp_T> ConstBiIter;
...
public:
    ...
    ConstBiIter search(const Elem_T& e) {
        Node* cur = _root;
        ...
        return _create_ConstBiIter(cur);               
    }  
    ...
    Elem_T remove(ConstBiIter iter) {
    ...
    ConstBiIter minimum_at_subtree(ConstBiIter iter) {
        const Node* cur = _extract_Node(iter);
        if (cur == _NIL) return _create_ConstBiIter(_NIL);
    ...
    ConstBiIter maximum_at_subtree(ConstBiIter iter) {
        const Node* cur = _extract_Node(iter);
        if (cur == _NIL) return _create_ConstBiIter(_NIL);
    ...
    ConstBiIter successor(ConstBiIter iter) {
        const Node* node = _extract_Node(iter);
        if (node == _NIL)
            return _create_ConstBiIter(_NIL);
        if (node->right != _NIL)
        ...
        if (node == _NIL)
            return _create_ConstBiIter(_NIL);
        return _create_ConstBiIter(node->p);
    }

    ConstBiIter predecessor(ConstBiIter iter) {
        const Node* node = _extract_Node(iter);
        if (node == _NIL)
            return _create_ConstBiIter(_NIL);
        if (node->left != _NIL)
...
private:
    ...
    ConstBiIter _create_ConstBiIter(const Node* node) {
        return {node, *this};
    }

    Node* _extract_Node(ConstBiIter iter) {
        return const_cast<Node*>(iter._node);       //Bad: ConstBiIter cast but...
    }
    Node* _root = _NIL;
    const Comp_T _less{};
...
public:
    ...
    bool operator==(const ConstBiIter& other) const {
        return _node == other._node;
    }
private:
    ConstBiIter(const Node* node, const RedBlackTree<Elem_T, Comp_T>& owner): _node{node}, _owner{owner} {}
    const Node* _node;
    const RedBlackTree<Elem_T, Comp_T>& _owner;
};

template <typename Elem_T, typename Comp_T>
struct std::iterator_traits<ConstBiIter<Elem_T, Comp_T>> {
